\frame{%
  \frametitle{Técnicas de modularização}
  \begin{block}
    \centering
  Tables são o único mecanismo de abstração
  Podem simular abstrações como:
  arrays, sequences, symbol tables, sets, records, graphs, trees.
  \end{block}
}

\frame{%
  \frametitle{Checagem de tipos}
  \begin{block}
    \centering
    \begin{itemize}
      \item Tipagem Dinâmica
      \item Valores nunca são tratados com tipo incorreto
      \item Verificação de tipos em tempo de execução
    \end{itemize}
  \end{block}
}

\section{Exemplos de Algoritmos}
\begin{frame}[fragile]
  \frametitle{Bubblesort}
  \begin{lstlisting}
  \end{lstlisting}
\end{frame}

\input{pass_par}
\input{tad}
\input{polimorfismo}
%\input{inf_tipos}
\input{sobrecarga}
%\input{heranca}
%\input{class_abstr}
%\input{excecoes}
